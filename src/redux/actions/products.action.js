...
export const getProductsByQuery = (
    ...
    dispatch({ type: actionsType.PRODUCTS_START })
    try {
        const response = await productApi.getProductsByQuery(name, category, minPrice, maxPrice, page, limit, sort, inStock)
        dispatch({ type: actionsType.GET_PRODUCT_BY_QUERY, payload: response.data });
    } catch (error) {
        // console.log(error);
        dispatch({ type: actionsType.PRODUCTS_FAIL, payload: error.response?.data });
    }
